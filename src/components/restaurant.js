...
await request(
    Platform.select({
      ...
      await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1002&type=restaurant&key=${API_KEY}`)
  ...
  handleRefresh = () => {
    this.setState(
      ...
      () => {
         axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=restaurant&key=${API_KEY}`)
        .then(response=>this.setState({
...
if (this.state.pageToken === undefined) {
 ...
 .then(response=>this.setState({
  ...
  axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=restaurant&key=${API_KEY}&pagetoken=${this.state.pageToken}`)
  .then(response=>this.setState({
    places: response.data.results,
    fetching:false,
    refreshing:false
