...
export default class Taksi extends Component {
	state = {
			region: {
			latitude: 36.726215,
			longitude: 27.685844,
...
await request(
    Platform.select({
      ...
      fetching:true
		...
		const { data: { results } } = await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${latitude},${longitude}&radius=1000&type=taxi_stand&key=${API_KEY}`)
      this.setState({
				places: results,
        fetching:false,
        refreshing:false
			});
      await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1002&type=taxi_stand&key=${API_KEY}`)
  ...
  handleRefresh = () => {
    this.setState(
      ...
      () => {
         axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=taxi_stand&key=${API_KEY}`)
        .then(response=>this.setState({
  ...
  handleLoadMore = () => {
    ...
    axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=taxi_stand&key=${API_KEY}&pagetoken=${this.state.pageToken}`)
   .then(response=>this.setState({
     places: response.data.results,
     fetching:false,
     refreshing:false
          ...
          const   destination={latitude:this.state.destination.latitude, longitude:this.state.destination.longitude}
               ...
               this.state.destination.check? console.log("bilgi yok") :
                ...
                <Callout  >
                  ...
                  {
            						this.state.fetching ? <Text style={styles.loading}>Yekındaki Mekanlar Alınıyor...</Text> : <Places map={this.map} places={this.state.places} />
