  ...
  async componentDidMount() {
    ...
    try {
      ...
      const {
        ...
        `${API_ENDPOINT}/nearbysearch/json?location=${latitude},${longitude}&radius=1000&type=cafe&key=${API_KEY}`,
      );
      this.setState({
        places: results,
        fetching: false,
      ...
      await axios
        .get(
          `${API_ENDPOINT}/nearbysearch/json?location=${
            this.state.region.latitude
          },${
            this.state.region.longitude
          }&radius=1002&type=cafe&key=${API_KEY}`,
        )
        .then(response =>
          this.setState({
        ...
        axios
          .get(
            `${API_ENDPOINT}/nearbysearch/json?location=${
              ...
              this.state.region.longitude
            }&radius=1003&type=cafe&key=${API_KEY}`,
          )
          .then(response =>
            this.setState({
      ...
      axios
        .get(
          `${API_ENDPOINT}/nearbysearch/json?location=${
            ...
            this.state.region.longitude
          }&radius=1003&type=cafe&key=${API_KEY}&pagetoken=${
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.placesContainer}>
          {this.state.fetching ? (
            <Text style={styles.loading}>Yekındaki Mekanlar Alınıyor...</Text>
          ) : (
            <Places
              map={this.map}
